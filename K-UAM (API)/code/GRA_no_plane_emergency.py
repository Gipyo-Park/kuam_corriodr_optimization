...
if __name__ == "__main__":
    file_path = '../data/high_res_affected_population_GRC.npy'
    try:
...
#--------------------------Refine Segments for Fine-Grained Risk------------------------------
    ...
    refined_segments_add = []
    refined_risks_add = []
    refined_xs_add, refined_ys_add, refined_zs_add = [], [], []
    
    start_dms_new = ('35°36\'12.01"N', '129°4\'39.69"E')
    mid_point_new = (1143000, 1732000) 
    end_dms_new   = ('35°34\'4.16"N',  '129°6\'24.12"E')

    lat_start = dms_str_to_decimal(start_dms_new[0])
    lon_start = dms_str_to_decimal(start_dms_new[1])
    x_start, y_start = transformer.transform(lon_start, lat_start)

    lat_end = dms_str_to_decimal(end_dms_new[0])
    lon_end = dms_str_to_decimal(end_dms_new[1])
    x_end, y_end = transformer.transform(lon_end, lat_end)

    z_start, z_mid, z_end = 500.0, 500.0, 500.0

    new_points = [(x_start, y_start, z_start),
                (mid_point_new[0], mid_point_new[1], z_mid),
                (x_end, y_end, z_end)]
    ...
    for (x1, y1, z1), (x2, y2, z2) in new_segments:
        p1 = np.array([x1, y1, z1])
        p2 = np.array([x2, y2, z2])
        segment_length = np.linalg.norm(p2 - p1)
        num_subsegments = max(1, int(segment_length // segment_detail_interval))
        ...
        for i in range(num_subsegments):
            ...
            refined_segments_add.append((tuple(pt_start), tuple(pt_end)))
            refined_risks_add.append(risk)

            refined_xs_add.append(pt_start[0])
            refined_ys_add.append(pt_start[1])
            refined_zs_add.append(pt_start[2])
    ...
    refined_risks_add = np.array(refined_risks_add)
    
    mean_refined = np.mean(refined_risks) if refined_risks.size > 0 else 0
    mean_refined_add = np.mean(refined_risks_add) if refined_risks_add.size > 0 else 0
...
#--------------------------Line Segment------------------------------
    ...
    for (x1, y1, z1), (x2, y2, z2), risk in zip(
        [s[0] for s in refined_segments_add],
        [s[1] for s in refined_segments_add],
        refined_risks_add):
        refined_lines_add.append([(x1, y1, z1), (x2, y2, z2)])
        refined_colors_add.append(risk)
        
    combined = np.concatenate([refined_risks, refined_risks_add])
        
    max_risk = np.max(combined)
    min_risk = np.min(combined)
    ...
    norm = plt.Normalize(min_risk, max_risk)
    # line_collection = Line3DCollection(lines, colors=cmap(norm(line_colors)), linewidths=3)
    
    refined_line_collection = Line3DCollection(refined_lines, colors=cmap(norm(refined_colors)), linewidths=5)
    refined_line_collection_add = Line3DCollection(refined_lines_add, colors=cmap(norm(refined_colors_add)), linewidths=5)
    
    # refined_line_collection = Line3DCollection(refined_lines, colors='white', linewidths=5)
    # refined_line_collection_add = Line3DCollection(refined_lines_add, colors='white', linewidths=5)
...
#----------------------------------------Add Map-----------------------------------------------
    ...
    def init():
        ...
        ax.add_collection3d(refined_line_collection_add)

        
        # ax.scatter(xs, ys, zs, c=risks, norm = norm, cmap='Reds', s=60)
        # ax.scatter(refined_xs, refined_ys, refined_zs, c=refined_risk_norm, cmap='Reds', s=pt_start_marker_size, marker='o')
        
        
        # ax.scatter(refined_xs, refined_ys, refined_zs, c=refined_risks, norm = norm, cmap='Reds', s=pt_start_marker_size, marker='o')      
        # ax.scatter(refined_xs_add, refined_ys_add, refined_zs_add,
        #    c=refined_risks_add, norm=norm, cmap='Reds',
        #    s=add_marker_size, marker='*')
        
        ax.scatter(refined_xs, refined_ys, refined_zs,
                color='white',
                s=pt_start_marker_size, marker='o')

        ax.scatter(refined_xs_add, refined_ys_add, refined_zs_add,
                color='yellow',
                s=pt_start_marker_size, marker='o')
        
        # ax.text2D(0.6, 0.6, f"Mean Risk: {mean_refined:.2f}",
        #     fontsize=8, color='black', weight='bold', ha='center', va='top',
        #     transform=ax.transAxes,
        #     bbox=dict(facecolor='white', alpha=1.0, edgecolor='none'))
            
            
                
        # ax.text2D(0.4, 0.4, f"Mean Risk: {mean_refined_add:.2f}",
        #     fontsize=8, color='black', weight='bold', ha='center', va='top',
        #     transform=ax.transAxes,
        #     bbox=dict(facecolor='white', alpha=1.0, edgecolor='none'))
                          
        print(f'origin : ',mean_refined,f'additional_path : ',mean_refined_add)
        ...
        ax.set_xlim(1142000, 1146000)
        ax.set_ylim(1732000, 1735000)
    ...
    def update(frame):
        ...
        if frame < len(refined_lines):
            ...
            ax.scatter(
                refined_xs[frame:], refined_ys[frame:], refined_zs[frame:],
                c=refined_risks[frame:], norm=norm, cmap='Reds',
    ...
    # sc = ax.scatter(xs, ys, zs, c=risk_norm, cmap='Reds', s=60)
    # sc = ax.scatter(refined_xs, refined_ys, refined_zs, c=refined_risk_norm, cmap='Reds', s=pt_start_marker_size, marker='o')
    # sc = ax.scatter(refined_xs, refined_ys, refined_zs, c=refined_risks, norm = norm, cmap='Reds', s=pt_start_marker_size, marker='o')
    ...
    # if not DIAGONAL_VIEW :
    #     anim.save("ground_risk_optimal.gif", writer='pillow', fps=2)

    
    # else: 
    #     anim.save("ground_risk_optimal_diagonal.gif", writer='pillow', fps=2)
    ...
    cbar_norm = mcolors.Normalize(vmin=min_risk, vmax=max_risk)
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=cbar_norm)
    sm.set_array([])
    plt.colorbar(sm, ax=ax, label='Ground Risk (0–1)')
    
    ax.set_xlabel('X (TM)')
    ax.set_ylabel('Y (TM)')
    ax.set_zlabel('Altitude')
    ax.set_title('3D Ground Risk Along Optimized Corridor Segments')
    plt.tight_layout()
