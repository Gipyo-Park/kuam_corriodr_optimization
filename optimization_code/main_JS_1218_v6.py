...
def attempt_run_once():
    ...
    min_seg_for_extra_nodes_m = 1000.0  # 이보다 짧은 WP간 세그먼트에는 extra node 생성 안 함
    ...
    min_clicked_waypoints = 2   # 클릭으로 입력할 WP의 최소 개수 (takeoff/landing 제외)
    ...
    preview_takeoff, preview_landing, _ = build_takeoff_landing(
        vertiport, angle_deg=25.0, alt_delta_m=350.0,
        takeoff_sector=6, landing_sector=11,
        takeoff_target_alt_m=waypoint_alt_fixed_m,
        landing_target_alt_m=waypoint_alt_fixed_m,
    )

    emergency_lat = np.array([35.6201083, 35.5678222, 35.5919889])
    emergency_lon = np.array([129.1191806, 129.106728, 129.0751972])
    emergency_alts_preview = np.full(emergency_lat.shape[0], waypoint_alt_fixed_m, dtype=float)
    emergency_points_preview = np.column_stack([emergency_lat, emergency_lon, emergency_alts_preview])
    ...
    params_dict.update({
        "waypoint_source": ("clicked_map" if clicked_wp_json_path is not None else "fallback_default"),
        ...
        "airspace_info": {
            ...
            "boundary_lla": [
                ...
                for p in build_circle_lla(vertiport, airspace_radius_m, n_pts=180)
            ],
        },
        "no_fly_zones": [
            {
    ...
    for _retry in range(1, max_init_retries + 1):
        ...
        _feasible_init = []
        if _rf_cnt > 0:
            for _idx, (_rf_ok, _rf_path) in enumerate(_rf_ok_list):
                if not _rf_ok:
                    continue
                ...
                if _cst_ok and _air_ok and _dist_ok:
                    _both_cnt += 1
                    _feasible_init.append(_candidate[_idx])
    ...
    for gh in gen_history:
        ...
        draw_vertiport_radius_rings(gxg, vertiport)
        ...
        gxg.scatter([vertiport[1]], [vertiport[0]], s=120, c="red", edgecolors="k",
                    marker="s", transform=ccrs.Geodetic(), label="Vertiport", zorder=7)
    ...
    if best_rep is not None:
        ...
        for pi, p in enumerate(build_circle_lla(vertiport, airspace_radius_m, n_pts=180), start=1):
