...
def build_takeoff_landing(vertiport, angle_deg=25.0, alt_delta_m=350.0,
    ...
    return np.array([to_lat, to_lon, alt0]), np.array([ld_lat, ld_lon, alt0]), d
...
def generate_single_initial_solution(
    ...
    for i in range(K):
        ...
        perturbed[i, 0] += d * r_lat * np.sin(ang)
        perturbed[i, 1] += d * r_lon * np.cos(ang)
...
def attempt_run_once():
    # ==================== 파라미터 ====================
    W_half = 95.0                   # TSE (m)
    ground_speed_mps = 70.65        # 254 km/h → 70.65m/s RF turn 시 속도 (m/s)
    ...
    look_ahead_min_scale = 0.5   # R 축소 하한 비율 (0.5 = 최대 50% 축소)
    ...
    waypoints = np.column_stack([corridor_lat, corridor_lon,
                                 np.full_like(corridor_lat, vertiport[2])])
    ...
    emergency_points = np.column_stack([emergency_lat, emergency_lon,
                                        np.full_like(emergency_lat, vertiport[2])])
    ...
    def _build_safe_nodes(a, b):
        ...
        ai = int(np.argmin(np.abs(altitude_levels - nodes_seg[0, 2])))
    ...
    # 여유를 둬서 경로가 잘리지 않게
    ext_margin = 0.008
    map_extent = [lon_lim[0] - ext_margin, lon_lim[1] + ext_margin,
                  lat_lim[0] - ext_margin, lat_lim[1] + ext_margin]
    ...
    fig1.subplots_adjust(left=0.05, right=0.78)
    ...
    # 세그먼트별 후보 노드 (색 구분)
    seg_colors = plt.cm.Set2(np.linspace(0, 1, len(safe_nodes_by_seg)))
    for ki, seg_nodes in enumerate(safe_nodes_by_seg):
        if seg_nodes.size > 0:
            lab = f"Seg {ki+1} nodes ({seg_nodes.shape[0]})" if ki < 3 else None
            gx1.scatter(seg_nodes[:, 1], seg_nodes[:, 0], s=4, c=[seg_colors[ki]],
                        alpha=0.9, transform=ccrs.Geodetic(), label=lab, zorder=3)
    ...
    if best_rep is not None:
        ...
        df.to_excel(str(xlsx_name), index=False, sheet_name="Routes Data")
