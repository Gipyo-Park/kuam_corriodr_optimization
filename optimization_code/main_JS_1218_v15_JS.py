...
from evaluate_objectives_with_constraints_GP import evaluate_objectives_with_constraints_gp as _eval_constraints_orig
from evaluate_objectives_uniform import evaluate_objectives_uniform
...
def evaluate_objectives_with_constraints_gp(
    ...
    lat_lim, lon_lim,
    NoiseRisk=None, noise_floor_db=0.0, w_noise=1.0,
    W_half=None, check_corridor_nfz=False,
    MOCRisk=None, check_corridor_moc=False,
    vertiport=None, landing_entry=None, takeoff_complete=None,
    return_reason=False,
    sample_spacing_m=50.0,
):
    """기존 constraint 체크는 원본 함수 사용, 목적함수 계산만 uniform 방식으로 교체."""
    # 목적함수 차원 결정 (NoiseRisk 유무에 따라 3 or 4)
    n_obj = 4 if (NoiseRisk is not None and np.size(NoiseRisk) > 0) else 3
    penalty_cost_array = np.full(n_obj, 1e6, dtype=float)

    def _pack(ret_f, ret_ok, reason_str):
        if return_reason:
            return ret_f, ret_ok, reason_str
        return ret_f, ret_ok

    # 0) path 기본 검증
    if path is None or np.asarray(path).shape[0] < 2:
        return _pack(penalty_cost_array, False, "path_too_short")

    # 1) constraint 체크 (원본 함수 호출) - feasibility만 사용, 목적함수 값은 버림
    orig_ret = _eval_constraints_orig(
        path, Norm_RT, AirRisk, use_heading_map,
        flight_dist_limit, forbidden_zones, delta_z_max,
        altitude_levels, cell_size, refine_scales,
        air_risk_threshold, w_dist, w_ground, w_air,
    ...
    _, feas, reason = orig_ret

    # 2) infeasible이면 penalty 반환
    if not feas:
        return _pack(penalty_cost_array, False, reason)

    # 3) feasible이면 uniform 방식으로 목적함수 계산
    #    vertiport 경로 추가는 원본과 동일하게 처리
    eval_path = np.asarray(path, dtype=float)
    if (vertiport is not None) and (landing_entry is not None) and (takeoff_complete is not None):
        eval_path = np.vstack([vertiport, takeoff_complete, eval_path, landing_entry, vertiport])

    f_val = evaluate_objectives_uniform(
        eval_path, Norm_RT, AirRisk, use_heading_map,
        altitude_levels, cell_size, air_risk_threshold,
        w_dist, w_ground, w_air, lat_lim, lon_lim,
        sample_spacing_m=sample_spacing_m,
        NoiseRisk=NoiseRisk, noise_floor_db=noise_floor_db, w_noise=w_noise,
    )

    return _pack(f_val, True, "ok")
...
def run_nsga3(
    ...
    for gen in range(1, Nmax + 1):
        ...
        if gen < Nmax:
            # 세대 진행률(0→1)에 따라 mutation 강도를 선형 증가
            progress = gen / Nmax  # 0.0 ~ 1.0
            adaptive_cfg = dict(mutation_cfg)
            adaptive_cfg["mutation_rate"] = mutation_cfg.get("mutation_rate", 0.2) * (0.5 + 1.0 * progress)
            adaptive_cfg["local_strip_width_m"] = mutation_cfg.get("local_strip_width_m", 500) * (0.5 + 1.0 * progress)
            adaptive_cfg["local_radius_m"] = mutation_cfg.get("local_radius_m", 500) * (0.5 + 1.0 * progress)
...
def attempt_run_once():
    ...
    Nmax = 50       # 최대 세대 수
    offspring_ratio = 1.0   # 자식 해 수 비율: n_off = round(len(parents) * offspring_ratio)
    ...
    local_strip_width_m = 1000.0          # 세그먼트 주변 strip 폭(m)
    local_radius_m = 1000.0               # 현재 변이 지점 주변 반경(m)
    ...
    for i, alt in enumerate(altitude_levels):
        src_idx = int(np.argmin(np.abs(bird_z_vec - float(alt))))
    ...
    for i, alt in enumerate(altitude_levels):
        src_idx = int(np.argmin(np.abs(moc_z_vec - float(alt))))
    ...
    # ==================== Backbone 경로 목적함수 값 ====================
    rf_bb = apply_rf_turns_full_corridor(
        backbone,
        start_vertiport,
        end_vertiport,
        ground_speed_mps,
    ...
    f_bb, feas_bb = evaluate_objectives_with_constraints_gp(
        rf_bb["path"], Norm_RT, AirRisk, use_heading_map,
        flight_dist_limit, forbidden_zones, delta_z_max,
        altitude_levels, cell_size, refine_scales,
    ...
    bb_val_str = ", ".join(
        f"{objective_names[j]}={f_bb[j]:.4f}" for j in range(len(objective_names))
    )
    ...
    if reps:
        ...
        # Balanced 목적함수 값 계산
        f_bal_val, _ = evaluate_objectives_with_constraints_gp(
            bal_full_path, Norm_RT, AirRisk, use_heading_map,
            flight_dist_limit, forbidden_zones, delta_z_max,
            altitude_levels, cell_size, refine_scales,
            air_thr_global, w_dist, w_ground, w_air, lat_lim, lon_lim,
            NoiseRisk=NoiseRisk, noise_floor_db=noise_floor_db, w_noise=w_noise,
        ...
        bal_val_str = ", ".join(
            f"{objective_names[j]}={f_bal_val[j]:.4f}" for j in range(len(objective_names))
        )
        # bal_val_str 은 마지막 요약 출력에서 사용
        ...
        gx5.set_title(f"Balanced Corridor (RF Turn applied)\n{bal_val_str}", fontsize=10)
        ...
        # ==================== [Fig 5-Gen1] 1세대 Balanced 회랑 ====================
        gen1_bal_val_str = None
        if gen_history and gen_history[0]["population"] and gen_history[0]["f_vals"].size > 0:
            gen1_reps = pick_representatives(gen_history[0]["population"], gen_history[0]["f_vals"])
            if gen1_reps:
                gen1_bal = gen1_reps[-1]
                rf_gen1 = apply_rf_turns_full_corridor(
                    gen1_bal,
                    start_vertiport, end_vertiport,
                    ground_speed_mps, bank_angle_deg, num_arc_points,
                    look_ahead, look_ahead_threshold_m, look_ahead_min_scale, look_ahead_window,
                )
                gen1_full_path = rf_gen1["path"]
                f_gen1_val, _ = evaluate_objectives_with_constraints_gp(
                    gen1_full_path, Norm_RT, AirRisk, use_heading_map,
                    flight_dist_limit, forbidden_zones, delta_z_max,
                    altitude_levels, cell_size, refine_scales,
                    air_thr_global, w_dist, w_ground, w_air, lat_lim, lon_lim,
                    NoiseRisk=NoiseRisk, noise_floor_db=noise_floor_db, w_noise=w_noise,
                ...
                gen1_bal_val_str = ", ".join(
                    f"{objective_names[j]}={f_gen1_val[j]:.4f}" for j in range(len(objective_names))
                )

                fig_g1 = plt.figure("Fig5-Gen1: Gen1 Balanced Corridor", figsize=(14, 10))
                fig_g1.subplots_adjust(left=0.05, right=0.72)
                gx_g1 = fig_g1.add_subplot(1, 1, 1, projection=request.crs)
                gx_g1.set_extent(map_extent)
                gx_g1.add_image(request, 13)
                gx_g1.set_title(f"Gen 1 Balanced Corridor (RF Turn applied)\n{gen1_bal_val_str}", fontsize=10)
                draw_vertiport_radius_rings(gx_g1, airspace_center_lla, radii_m=(airspace_radius_m,))
                plot_forbidden_zones(gx_g1, forbidden_zones, face_alpha=0.10, edge_alpha=0.80)
                plot_moc_binary_overlay(gx_g1, moc_plot_2d, lat_lim, lon_lim,
                                        label="MOC=1 (Corridor-Prohibited)",
                                        fill_color="magenta", fill_alpha=0.18)
                gx_g1.plot(bb_full[:, 1], bb_full[:, 0], "r--", linewidth=1.2, transform=ccrs.Geodetic(),
                           label="Backbone", zorder=4)
                gx_g1.scatter(waypoints[:, 1], waypoints[:, 0], s=55, c="orange", edgecolors="k",
                              linewidths=0.5, marker="o", transform=ccrs.Geodetic(), label="Waypoints", zorder=6)
                gx_g1.scatter([start_vertiport[1]], [start_vertiport[0]], s=120, c="red", edgecolors="k",
                              marker="s", transform=ccrs.Geodetic(), label="Start Vertiport", zorder=7)
                gx_g1.scatter([end_vertiport[1]], [end_vertiport[0]], s=120, c="crimson", edgecolors="k",
                              marker="D", transform=ccrs.Geodetic(), label="End Vertiport", zorder=7)
                gx_g1.scatter([takeoff_complete[1]], [takeoff_complete[0]], s=90, c="blue",
                              marker="^", transform=ccrs.Geodetic(), label="Takeoff", zorder=7)
                gx_g1.scatter([landing_entry[1]], [landing_entry[0]], s=90, c="green",
                              marker="v", transform=ccrs.Geodetic(), label="Landing", zorder=7)

                plot_corridor_width(gx_g1, gen1_full_path, W_half, color="darkgreen", alpha=0.14)
                for seg in rf_gen1["segments"]:
                    pts = seg["points"]
                    if seg["type"] == "TF":
                        gx_g1.plot(pts[:, 1], pts[:, 0], "-", color="darkgreen", linewidth=1.8,
                                   transform=ccrs.Geodetic(), zorder=8)
                    else:
                        gx_g1.plot(pts[:, 1], pts[:, 0], "-", color="darkgreen", linewidth=2.8,
                                   transform=ccrs.Geodetic(), zorder=9)
                        gx_g1.scatter(pts[0, 1], pts[0, 0], s=40, c="yellow", marker=">",
                                      edgecolors="k", linewidths=0.6, transform=ccrs.Geodetic(), zorder=10)
                        gx_g1.scatter(pts[-1, 1], pts[-1, 0], s=40, c="yellow", marker="s",
                                      edgecolors="k", linewidths=0.6, transform=ccrs.Geodetic(), zorder=10)
                        ac = seg["arc_center"]
                        gx_g1.scatter(ac[1], ac[0], s=50, c="white", marker="x",
                                      linewidths=1.5, transform=ccrs.Geodetic(), zorder=10)

                gx_g1.plot([], [], "-", color="darkgreen", linewidth=1.8, label="Gen1 Balanced (TF)")
                gx_g1.plot([], [], "-", color="darkgreen", linewidth=2.8, label="Gen1 Balanced (RF arc)")
                gx_g1.scatter([], [], s=40, c="yellow", marker=">", edgecolors="k", label="Arc Start")
                gx_g1.scatter([], [], s=40, c="yellow", marker="s", edgecolors="k", label="Arc End")
                gx_g1.scatter([], [], s=50, c="white", marker="x", linewidths=1.5, label="Arc Center")
                gx_g1.legend(loc="center left", bbox_to_anchor=(1.01, 0.5), fontsize=7, framealpha=0.9)
                fig_g1_path = out_dir / "fig5_gen1_balanced.png"
                fig_g1.savefig(fig_g1_path, dpi=150, bbox_inches="tight")
                print(f"Saved {fig_g1_path}")
                plt.close(fig_g1)

        # ==================== [Fig 5A] 각 목적함수별 최적 회랑 개별 그림 ====================
        # reps[0..n_obj-1] = 각 목적함수 최적 해, reps[-1] = Balanced
        obj_best_val_strs = []
        obj_best_colors = [plt.cm.tab10(i % 10) for i in range(len(objective_names))]
        for oi in range(len(objective_names)):
            if oi >= len(reps) - 1:
                break
            obj_rep = reps[oi]
            obj_name = objective_names[oi]
            obj_col = obj_best_colors[oi]
            safe_name = obj_name.replace(" ", "_").lower()

            rf_obj = apply_rf_turns_full_corridor(
                obj_rep,
                start_vertiport,
                end_vertiport,
                ground_speed_mps,
            ...
            obj_full_path = rf_obj["path"]

            # 목적함수 값 계산
            f_obj_val, _ = evaluate_objectives_with_constraints_gp(
                obj_full_path, Norm_RT, AirRisk, use_heading_map,
                flight_dist_limit, forbidden_zones, delta_z_max,
                altitude_levels, cell_size, refine_scales,
                air_thr_global, w_dist, w_ground, w_air, lat_lim, lon_lim,
                NoiseRisk=NoiseRisk, noise_floor_db=noise_floor_db, w_noise=w_noise,
            ...
            # 목적함수 값 텍스트
            obj_val_str = ", ".join(
                f"{objective_names[j]}={f_obj_val[j]:.4f}" for j in range(len(objective_names))
            )
            obj_best_val_strs.append((obj_name, obj_val_str))

            fig_obj = plt.figure(f"Fig5A: Best {obj_name} Corridor", figsize=(14, 10))
            fig_obj.subplots_adjust(left=0.05, right=0.72)
            gx_obj = fig_obj.add_subplot(1, 1, 1, projection=request.crs)
            gx_obj.set_extent(map_extent)
            gx_obj.add_image(request, 13)
            gx_obj.set_title(f"Best {obj_name} Corridor (RF Turn applied)\n{obj_val_str}", fontsize=10)
            draw_vertiport_radius_rings(gx_obj, airspace_center_lla, radii_m=(airspace_radius_m,))
            plot_forbidden_zones(gx_obj, forbidden_zones, face_alpha=0.10, edge_alpha=0.80)
            plot_moc_binary_overlay(gx_obj, moc_plot_2d, lat_lim, lon_lim,
                                    label="MOC=1 (Corridor-Prohibited)",
                                    fill_color="magenta", fill_alpha=0.18)

            gx_obj.plot(bb_full[:, 1], bb_full[:, 0], "r--", linewidth=1.2, transform=ccrs.Geodetic(),
                        label="Backbone", zorder=4)
            gx_obj.scatter(waypoints[:, 1], waypoints[:, 0], s=55, c="orange", edgecolors="k",
                           linewidths=0.5, marker="o", transform=ccrs.Geodetic(), label="Waypoints", zorder=6)
            gx_obj.scatter([start_vertiport[1]], [start_vertiport[0]], s=120, c="red", edgecolors="k",
                           marker="s", transform=ccrs.Geodetic(), label="Start Vertiport", zorder=7)
            gx_obj.scatter([end_vertiport[1]], [end_vertiport[0]], s=120, c="crimson", edgecolors="k",
                           marker="D", transform=ccrs.Geodetic(), label="End Vertiport", zorder=7)
            gx_obj.scatter([takeoff_complete[1]], [takeoff_complete[0]], s=90, c="blue",
                           marker="^", transform=ccrs.Geodetic(), label="Takeoff", zorder=7)
            gx_obj.scatter([landing_entry[1]], [landing_entry[0]], s=90, c="green",
                           marker="v", transform=ccrs.Geodetic(), label="Landing", zorder=7)

            plot_corridor_width(gx_obj, obj_full_path, W_half, color=obj_col, alpha=0.14)
            for seg in rf_obj["segments"]:
                pts = seg["points"]
                if seg["type"] == "TF":
                    gx_obj.plot(pts[:, 1], pts[:, 0], "-", color=obj_col, linewidth=1.8,
                                transform=ccrs.Geodetic(), zorder=8)
                else:
                    gx_obj.plot(pts[:, 1], pts[:, 0], "-", color=obj_col, linewidth=2.8,
                                transform=ccrs.Geodetic(), zorder=9)
                    gx_obj.scatter(pts[0, 1], pts[0, 0], s=40, c="yellow", marker=">",
                                   edgecolors="k", linewidths=0.6, transform=ccrs.Geodetic(), zorder=10)
                    gx_obj.scatter(pts[-1, 1], pts[-1, 0], s=40, c="yellow", marker="s",
                                   edgecolors="k", linewidths=0.6, transform=ccrs.Geodetic(), zorder=10)
                    ac = seg["arc_center"]
                    gx_obj.scatter(ac[1], ac[0], s=50, c="white", marker="x",
                                   linewidths=1.5, transform=ccrs.Geodetic(), zorder=10)

            gx_obj.plot([], [], "-", color=obj_col, linewidth=1.8, label=f"Best {obj_name} (TF)")
            gx_obj.plot([], [], "-", color=obj_col, linewidth=2.8, label=f"Best {obj_name} (RF arc)")
            gx_obj.scatter([], [], s=40, c="yellow", marker=">", edgecolors="k", label="Arc Start")
            gx_obj.scatter([], [], s=40, c="yellow", marker="s", edgecolors="k", label="Arc End")
            gx_obj.scatter([], [], s=50, c="white", marker="x", linewidths=1.5, label="Arc Center")
            gx_obj.legend(loc="center left", bbox_to_anchor=(1.01, 0.5), fontsize=7, framealpha=0.9)
            fig_obj_path = out_dir / f"fig5a_best_{safe_name}.png"
            fig_obj.savefig(fig_obj_path, dpi=150, bbox_inches="tight")
            print(f"Saved {fig_obj_path}")
            plt.close(fig_obj)
        ...
        gx5b.set_title(f"Balanced Corridor (Fig4 Style)\n{bal_val_str}", fontsize=10)
    ...
    # ==================== 목적함수 값 요약 출력 ====================
    print("\n" + "=" * 70)
    print("  Objective Cost Summary")
    print("=" * 70)
    print(f"  [Backbone]          {bb_val_str}")
    if reps and gen1_bal_val_str is not None:
        print(f"  [Gen 1 Balanced]    {gen1_bal_val_str}")
    print("-" * 70)
    if reps:
        for obj_name_s, obj_str_s in obj_best_val_strs:
            print(f"  [Best {obj_name_s:<13s}] {obj_str_s}")
        print(f"  [Final Balanced]    {bal_val_str}")
    print("=" * 70 + "\n")
    ...
    if best_rep is not None:
            ...
            if stype == "TF":
                for pi in range(pts.shape[0]):
                    ...
                    rows.append({
                        ...
                        "Lat": cur_pt[0], "Lon": cur_pt[1], "Alt_m": cur_pt[2],
        ...
        if not df.empty:
            ...
            def _nearest_index_and_distance(df_in, ref_point):
                pts = df_in[["Lat", "Lon", "Alt_m"]].to_numpy(dtype=float)
            ...
            if i_ld is not None and i_end - i_ld > 1:
                ...
                if last_landing_rf_idx is not None:
                    last_rf_point = df.loc[last_landing_rf_idx, ["Lat", "Lon", "Alt_m"]].values
                    landing_end_row = {
                        "Point_No": -1,
        ...
        route_points = df[["Lat", "Lon", "Alt_m"]].to_numpy(dtype=float)
        ...
        route_col_order = [
            "Point_No", "Type", "Segment", "Lat", "Lon", "Alt_m",
        ...
        df = df[[c for c in route_col_order if c in df.columns]]

        xlsx_name = out_dir / "route_data.xlsx"
        with pd.ExcelWriter(str(xlsx_name)) as writer:
            df.to_excel(writer, index=False, sheet_name="Route_Data")
