...
if str(PROJECT_ROOT) not in _api_sys.path:
    _api_sys.path.insert(0, str(PROJECT_ROOT))
...
def attempt_run_once(
    ...
    if cruise_altitude_m is None:
        if len(corridor_points) > 0 and len(corridor_points[0]) >= 3:
            cruise_altitude_m = float(corridor_points[0][2])
        else:
            cruise_altitude_m = 750.0
    altitude_levels = np.array([float(cruise_altitude_m)], dtype=float)
    ...
    use_clicked_waypoints = False    # API engine uses request waypoints instead of click UI
    ...
    ground_risk_path = project_path("ground_risk_data", "Modified_high_res_affected_population_GRC.npy")

    bird_airrisk_path = project_path("air_risk_data", "bird_riskmap_springfall_3d.npy")
    moc_airrisk_dir = project_path("260608_MOC")
    lat_lim = [35.535, 35.652]
    lon_lim = [129.020, 129.150]

    import datetime as _dt
    _run_ts = _dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = project_path("runs") / _run_ts
    ...
    if end_point is not None:
        end_vertiport_default = np.asarray(end_point, dtype=float).reshape(3)
...
def _parse_corridor_points(raw_points: Optional[Sequence[Any]], default_alt_m: float) -> np.ndarray:
    ...
    for idx, p in enumerate(raw_points, start=1):
        ...
        raise ValueError(f"Invalid corridor_points[{idx}] type. Use dict {{lat, lon, alt_m?}} or list [lat, lon, alt?].")
...
def _parse_no_fly_zones_to_bbox(no_fly_zones: Optional[Sequence[Any]]) -> np.ndarray:
    ...
    for idx, zone in enumerate(no_fly_zones, start=1):
        if isinstance(zone, (list, tuple, np.ndarray)):
            ...
            if len(vals) != 4:
                raise ValueError(f"Invalid no_fly_zones[{idx}] list length {len(vals)}. Use [lon_min, lon_max, lat_min, lat_max].")
        ...
        if isinstance(bbox, (list, tuple, np.ndarray)):
            ...
            if len(vals) != 4:
                raise ValueError(f"Invalid bbox list in no_fly_zones[{idx}]. Use [lon_min, lon_max, lat_min, lat_max].")
            boxes.append([float(vals[0]), float(vals[1]), float(vals[2]), float(vals[3])])
            continue

        if isinstance(bbox, dict):
            try:
                boxes.append([float(bbox["lon_min"]), float(bbox["lon_max"]), float(bbox["lat_min"]), float(bbox["lat_max"])])
            except Exception as e:
...
def _load_risk_maps():
    ...
    if not ground_path.exists():
        raise FileNotFoundError(f"Ground risk map not found: {ground_path}")
    if not bird_path.exists():
        raise FileNotFoundError(f"Bird risk map not found: {bird_path}")
    if not moc_dir.exists():
        raise FileNotFoundError(f"MOC directory not found: {moc_dir}")
    if not noise_path.exists():
        raise FileNotFoundError(f"Noise grid not found: {noise_path}")
    ...
    for attempt in range(1, int(max(1, max_attempts)) + 1):
        ...
        if ok:
            ...
            if pkl_candidates:
                ...
                all_pkls = [p for p in runs_dir.glob("*/results.pkl") if p.is_file()]
                if all_pkls:
