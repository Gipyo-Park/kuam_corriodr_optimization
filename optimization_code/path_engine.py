...
def _sample_point_risks(path, Norm_RT, AirRisk, altitude_levels,
    ...
    💡 현재: path의 모든 고도는 "해발고도(MSL: Mean Sea Level)" 기준
...
def _sample_point_noise(path, NoiseMap, altitude_levels, lat_lim, lon_lim):
    ...
    💡 현재: 소음맵은 지표면 기준 (고도 무관)
           path의 고도는 "해발고도(MSL: Mean Sea Level)" 기준이지만, 소음 평가에는 영향 없음
...
def _normalize_points(points: Optional[Sequence[Sequence[float]]]) -> Optional[np.ndarray]:
    """Normalize optional point arrays into (N, 3) float ndarray."""
    if points is None:
        return None
    ...
    if arr.ndim == 1:
        arr = arr.reshape(1, -1)
    if arr.shape[1] != 3:
        raise ValueError("Each point must be [lat, lon, alt].")
    return arr


def _parse_lla_dict(lla_obj: Dict[str, Any], field_name: str) -> np.ndarray:
    """Parse one LLA object {lat, lon, alt_m} into ndarray([lat, lon, alt])."""
    try:
        lat = float(lla_obj["lat"])
        lon = float(lla_obj["lon"])
        alt = float(lla_obj["alt_m"])
    except Exception as e:
        raise ValueError(f"Invalid {field_name}. Expected {{lat, lon, alt_m}}.") from e
    return np.array([lat, lon, alt], dtype=float)


def _parse_corridor_points(
    raw_points: Optional[Sequence[Any]],
    default_alt_m: float,
) -> np.ndarray:
    ...
    if not isinstance(raw_points, (list, tuple, np.ndarray)):
        raise ValueError("corridor_points/waypoints must be a list.")

    parsed: List[List[float]] = []
    for idx, p in enumerate(raw_points, start=1):
        if isinstance(p, dict):
            try:
                ...
                raise ValueError(
                    f"Invalid corridor_points[{idx}] dict. Expected {{lat, lon, alt_m?}}."
                ) from e
            parsed.append([lat, lon, alt])
            continue

        if isinstance(p, (list, tuple, np.ndarray)):
            vals = np.asarray(p, dtype=float).ravel().tolist()
            if len(vals) == 2:
            ...
            if len(vals) == 3:
                ...
                continue
            raise ValueError(
                f"Invalid corridor_points[{idx}] list length {len(vals)}. Use [lat, lon] or [lat, lon, alt]."
            )

        raise ValueError(
            f"Invalid corridor_points[{idx}] type. Use dict {{lat, lon, alt_m?}} or list [lat, lon, alt?]."
        )

    if not parsed:
        return np.empty((0, 3), dtype=float)
    return np.asarray(parsed, dtype=float).reshape(-1, 3)


def _parse_no_fly_zones_to_bbox(no_fly_zones: Optional[Sequence[Any]]) -> np.ndarray:
    """Parse no-fly zones and normalize into bbox array: [lon_min, lon_max, lat_min, lat_max]."""
    if not no_fly_zones:
        return np.empty((0, 4), dtype=float)

    boxes: List[List[float]] = []
    for idx, zone in enumerate(no_fly_zones, start=1):
        if isinstance(zone, (list, tuple, np.ndarray)):
            vals = np.asarray(zone, dtype=float).ravel().tolist()
            if len(vals) != 4:
                raise ValueError(
                    f"Invalid no_fly_zones[{idx}] list length {len(vals)}. Use [lon_min, lon_max, lat_min, lat_max]."
                )
            boxes.append([float(vals[0]), float(vals[1]), float(vals[2]), float(vals[3])])
            continue

        if not isinstance(zone, dict):
            raise ValueError(f"Invalid no_fly_zones[{idx}] format.")

        bbox = zone.get("bbox")
        if isinstance(bbox, (list, tuple, np.ndarray)):
            vals = np.asarray(bbox, dtype=float).ravel().tolist()
            if len(vals) != 4:
                raise ValueError(
                    f"Invalid bbox list in no_fly_zones[{idx}]. Use [lon_min, lon_max, lat_min, lat_max]."
                )
            boxes.append([float(vals[0]), float(vals[1]), float(vals[2]), float(vals[3])])
            continue
        ...
        if isinstance(bbox, dict):
            try:
                ...
                raise ValueError(f"Invalid bbox in no_fly_zones[{idx}].") from e
            continue

        center = zone.get("center")
        if not isinstance(center, dict):
            raise ValueError(f"no_fly_zones[{idx}] must include 'center' or 'bbox'.")

        try:
            c_lat = float(center["lat"])
            c_lon = float(center["lon"])
            r_km = float(zone.get("radius_km", 1.0))
        except Exception as e:
            raise ValueError(f"Invalid center/radius in no_fly_zones[{idx}].") from e

        r_m = max(0.0, r_km) * 1000.0
        m_lat = 111000.0
        m_lon = 111000.0 * np.cos(np.deg2rad(c_lat))
        d_lat = r_m / m_lat
        d_lon = r_m / m_lon if m_lon > 1e-9 else d_lat
        boxes.append([c_lon - d_lon, c_lon + d_lon, c_lat - d_lat, c_lat + d_lat])
...
def attempt_run_once(
    ...
    min_corridor_distance_km = 0.0 if min_corridor_distance_km is None else float(min_corridor_distance_km)
    ...
    airspace_radius_km = float(airspace_info.get("radius_km", 5.0))
    ...
    default_start_point = [35.6033361, 129.0776917, 150.0]
    default_end_point = [35.6033361, 129.0776917, 150.0]
    resolved_start_point = start_point if (start_point is not None and len(start_point) >= 3) else default_start_point
    resolved_end_point = end_point if (end_point is not None and len(end_point) >= 3) else default_end_point
    if cruise_altitude_m is not None:
        cruise_altitude_m = float(cruise_altitude_m)
    elif len(corridor_points) > 0:
        cruise_altitude_m = float(corridor_points[0][2])
    elif resolved_start_point is not None and len(resolved_start_point) >= 3:
        cruise_altitude_m = float(resolved_start_point[2])
    else:
        cruise_altitude_m = 750.0
    altitude_levels = np.array([cruise_altitude_m], dtype=float)
    ...
    start_ref_alt_m = float(resolved_start_point[2])
    ...
    start_vertiport = np.array(resolved_start_point, dtype=float)
    end_vertiport = np.array(resolved_end_point, dtype=float)
    ...
    corridor_points_arr = _normalize_points(corridor_points)
    if corridor_points_arr is not None and corridor_points_arr.size > 0:
    ...
    use_forbidden_zones = forbidden_zones_input.shape[0] > 0
    ...
    if reps:
        ...
        agl_value_5b = float(altitude_levels[0] - start_vertiport[2])
        title_str_5b = f"Balanced Corridor\nAltitude: {float(altitude_levels[0]):.1f}m MSL (= {agl_value_5b:.1f}m AGL above vertiport)"
        gx5b.set_title(title_str_5b)
    ...
    # ==================== 최적화된 회랑 데이터 정규화 ====================
    # 주의: 다음의 3가지 데이터는 모두 동일한 최적화된 회랑을 나타냅니다:
    # 1. excel_corridor_data: 엑셀 파일(route_data.xlsx)의 Route_Data 시트
    # 2. json_waypoints: API 응답 JSON의 waypoints 배열
    # 3. response_data: 클라이언트가 수신하는 응답 데이터
    # 
    # 이들의 point_count가 동일함을 보장: len(excel_corridor_data) == waypoint_count == len(response_data)
    # 
    # 엑셀이 가장 완전한 정보를 포함하고(Type, Segment, Risk, Distance, TF/RF markers, Arc center),
    # JSON response는 엑셀의 핵심 항목(Lat, Lon, Alt_m)만을 순서대로 전달합니다.
    excel_corridor_data = [
        {
            "lat": float(row["Lat"]),
            "lon": float(row["Lon"]),
            "alt_m": float(row["Altitude_MSL_m"]),
        }
    ...
    result = {
        ...
        "MOCRisk": MOCRisk,
        # ==================== 최적화된 회랑 경로 데이터 ====================
        # 다음 항목은 JSON 응답 생성 시 직접 사용됩니다:
        # - 엑셀 Route_Data 시트와 완전히 동일한 Transition 필터링 적용
        # - 동일한 TF/RF 경계 중복 제거 적용
        # - waypoint_count = len(excel_corridor_llas)로 정확하게 계산
        "excel_corridor_llas": excel_corridor_data,  # [{"lat", "lon", "alt_m"}, ...]
        "excel_corridor_point_count": len(df_excel),  # 엑셀 포인트 개수 (Transitions 제외)
    }
...
def _load_risk_maps():
    """Warm-up loader for API server startup compatibility.

    Ground risk map is intentionally fixed to the Modified file only.
    """
    ground_path = Path("Modified_high_res_affected_population_GRC.npy")
    if not ground_path.exists():
        raise FileNotFoundError(
            "Ground risk map not found: Modified_high_res_affected_population_GRC.npy"
        )
    np.load(str(ground_path), allow_pickle=True)
    np.load(Path("air_risk_data") / "bird_riskmap_springfall_3d.npy", allow_pickle=True)
    np.load(Path("air_risk_data") / "UAM_MOC_3D_Risk_Map.npy", allow_pickle=True)


def _best_full_path_from_result(result: Dict[str, Any]) -> List[List[float]]:
    """Build final full-path waypoints from a run result using balanced representative path."""
    reps = result.get("representative_paths") or []
    pop = result.get("population") or []
    best = reps[-1] if reps else (pop[0] if pop else None)
    if best is None:
        return []

    rf_best = apply_rf_turns_full_corridor(
        best,
        result["start_vertiport"],
        result["end_vertiport"],
        float(result["ground_speed_mps"]),
        float(result["bank_angle_deg"]),
        int(result.get("num_arc_points", 30)),
        bool(result.get("look_ahead", True)),
        float(result.get("look_ahead_threshold_m", 2000.0)),
        float(result.get("look_ahead_min_scale", 0.11)),
        int(result.get("look_ahead_window", 2)),
    )
    full_path = rf_best["path"].astype(float)
    return full_path.tolist()


def _unpack_request_payload(path_request: Dict[str, Any]) -> Dict[str, Any]:
    """Validate and unpack API payload into engine-ready primitive fields."""
    ...
    try:
        cruise_altitude_m = float(cruise_altitude_m_raw)
    except Exception as e:
...
def run_path_engine(
    ...
    for attempt in range(1, int(max(1, max_attempts)) + 1):
        before = {p.resolve() for p in Path("runs").glob("*") if p.is_dir()}
        ...
        after = [p.resolve() for p in Path("runs").glob("*") if p.is_dir()]
        new_dirs = sorted([p for p in after if p not in before], key=lambda p: p.name)
        if new_dirs:
            last_run_dir = Path(new_dirs[-1])
        elif after:
            last_run_dir = Path(sorted(after, key=lambda p: p.name)[-1])

        result_obj = None
        result_pkl: Optional[Path] = None
        if ok:
            # Prefer artifacts created in this attempt to avoid mixing with older run folders.
            candidate_dirs = [Path(p) for p in new_dirs] if new_dirs else ([] if last_run_dir is None else [last_run_dir])
            pkl_candidates = [d / "results.pkl" for d in candidate_dirs if (d / "results.pkl").exists()]
            if pkl_candidates:
                result_pkl = max(pkl_candidates, key=lambda p: p.stat().st_mtime)
            else:
                # Fallback: most recently written results.pkl in runs.
                all_pkls = [p for p in Path("runs").glob("*/results.pkl") if p.is_file()]
                if all_pkls:
                    result_pkl = max(all_pkls, key=lambda p: p.stat().st_mtime)

            if result_pkl is not None:
                last_run_dir = result_pkl.parent
                with open(result_pkl, "rb") as f:
                    result_obj = pickle.load(f)

            # ==================== 최적화된 회랑 경로 데이터 추출 ====================
            # 중요: 다음 데이터들은 모두 동일한 최적화된 회랑을 나타냅니다:
            # - excel_corridor_llas: 엑셀 Route_Data 시트에서 읽은 회랑 데이터 (Transitions 제외)
            # - 이것이 JSON response의 waypoints 소스가 됩니다
            # - waypoint_count = len(excel_corridor_llas)로 정확하게 계산됩니다
            excel_corridor = (result_obj.get("excel_corridor_llas", []) 
                            if result_obj is not None else [])
            excel_point_count = (result_obj.get("excel_corridor_point_count", 0) 
                               if result_obj is not None else 0)

            # Stop immediately when engine reports success.
            return {
                "success": True,
                "attempt": attempt,
                "feasible_count": int(feas),
                "run_dir": (str(last_run_dir) if last_run_dir is not None else None),
                # ==================== 최적화된 회랑을 JSON 응답으로 사용 ====================
                # optimal_path: 엑셀 Route_Data와 동일한 회랑 데이터
                #   형식: [[lat, lon, alt_m], [lat, lon, alt_m], ...]
                #   특징: Transition 포인트 제거, TF/RF 경계 중복 제거 완료
                #   개수: 항상 excel_point_count와 일치
                "optimal_path": [
                    [pt["lat"], pt["lon"], pt["alt_m"]] for pt in excel_corridor
                ] if excel_corridor else [],
                "result": result_obj,
                "artifact_files": (
                    sorted([str(p) for p in last_run_dir.glob("**/*") if p.is_file()])
                    if last_run_dir is not None and last_run_dir.exists()
                    else []
                ),
            }
    ...
    return {
        "success": False,
        "attempt": int(max(1, max_attempts)),
        "feasible_count": 0,
        "run_dir": (str(last_run_dir) if last_run_dir is not None else None),
        "optimal_path": [],
        "result": None,
