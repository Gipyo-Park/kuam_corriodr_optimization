...
def _size_transition_horizontal_distance(
    ...
    쉬운 해석:
    - L_A: '기울기 제한'으로 계산한 최소 밑변 길이
      (삼각형에서 세로변=고도차, 밑변=수평거리)
    - L_B: '상승/하강률 제한'으로 계산한 최소 밑변 길이
      (같은 고도차를 안전한 수직속도로 처리하려면 필요한 수평거리)
    - 최종은 L_req = max(L_A, L_B)
      -> 두 조건을 동시에 만족하려면 더 긴 쪽을 써야 함.
    """
...
def attempt_run_once():
    ...
    rf_allow_tangent_clamp = True  # True: 접선거리 초과 시 clamp 허용, False: 무클램프 엄격 판정
    rf_corner_fit_margin = 0.95    # 코너가 인접 세그먼트에서 사용할 수 있는 최대 비율(0~1)
    rf_corner_min_tangent_m = 1.0  # 코너 접선거리 최소값(m)
    rf_min_turn_angle_deg = 0.5    # 이 각도 미만은 직진으로 간주(도)
    ...
    # ── Transition(이착륙 전이) 3D 직선 프로파일 파라미터 ──
    # heading은 사용자가 직접 지정 가능(도 단위). 기본값은 기존 sector 방향과 동일하게 세팅.
    takeoff_heading_deg = float(np.rad2deg(_sector_angle(6)))
    landing_heading_deg = float(np.rad2deg(_sector_angle(11)))

    # 쉬운 해석:
    # - max_*_angle_deg: 삼각형 기울기(고도차/수평거리) 제한
    # - max_*_rate_mps: 초당 수직 이동량(상승률/하강률) 제한
    # 두 조건에서 계산한 최소 수평거리 L_A, L_B 중 큰 값을 사용한다.
    transition_max_climb_angle_deg = 50.0
    transition_max_descent_angle_deg = 50.0
    transition_max_climb_rate_mps = 50.0
    transition_max_descent_rate_mps = 50.0
    transition_horiz_speed_takeoff_mps = 50.0
    transition_horiz_speed_landing_mps = 50.0

    # None이면 자동(L_req) 사용. 값 지정 시 strict=False면 부족한 값은 자동 보정(clamp).
    transition_takeoff_distance_m = None
    transition_landing_distance_m = None
    transition_strict_distance_check = False

    # 전이 구간 3D 점 샘플 간격. 작을수록 경로는 부드럽고 점 수/계산량은 증가.
    ...
    # 전이 끝 고도는 운항고도(waypoint_alt_fixed_m)와 동일하게 강제한다.
    ...
    # 전이 시작/끝점은 위에서 sizing(L_A/L_B) 기반으로 계산한 값을 그대로 사용.
    # takeoff_complete / landing_entry의 고도는 항상 운항고도(= waypoint_alt_fixed_m)와 동일.
    ...
    for _retry in range(1, max_init_retries + 1):
        ...
        for _c in _candidate:
            ...
            _rf_ok_list.append((_rf["feasible"], bool(_rf.get("had_clamp", False)), _rf["path"], _c_eval))
        _rf_cnt = sum(1 for ok, _, _, _ in _rf_ok_list if ok)
        _rf_no_clamp_cnt = sum(1 for ok, had_clamp, _, _ in _rf_ok_list if ok and (not had_clamp))
        ...
        _feasible_init = []
        if _rf_cnt > 0:
            for _idx, (_rf_ok, _had_clamp, _rf_path, _c_eval) in enumerate(_rf_ok_list):
                if not _rf_ok:
                    continue
                full_path = _rf_path
    ...
    def _apply_rf_corridor(path_core):
    ...
    for si in range(n_sample):
        rf = _apply_rf_corridor(init_pop[si])
        rf_path = rf["path"]
    ...
    if init_pop:
        ...
        for i_init, p_init in enumerate(init_pop):
            rf_init = _apply_rf_corridor(p_init)
    ...
    if len(reps) > 3:
        ...
        for config in risk_configs:
            ...
            rf_risk = _apply_rf_corridor(risk_rep)
            ...
            gx_risk.plot([], [], "-", color="dodgerblue", linewidth=1.2, label="Takeoff Transition")
            gx_risk.plot([], [], "-", color="seagreen", linewidth=1.2, label="Landing Transition")
    ...
    if reps:
        balanced_rep = reps[-1]
        rf_bal = _apply_rf_corridor(balanced_rep)
        ...
        gx5.plot([], [], "-", color="dodgerblue", linewidth=1.5, label="Takeoff Transition")
        gx5.plot([], [], "-", color="seagreen", linewidth=1.5, label="Landing Transition")
        ...
        gx5b.scatter([takeoff_complete[1]], [takeoff_complete[0]], s=90, c="blue",
                     marker="^", transform=ccrs.Geodetic(), label="Takeoff_End", zorder=10)
        gx5b.scatter([landing_entry[1]], [landing_entry[0]], s=90, c="green",
                     marker="v", transform=ccrs.Geodetic(), label="Landing_Start", zorder=10)

        # legend 표시(요청사항)
        gx5b.plot([], [], "-", color="black", linewidth=1.8, label="Balanced (TF)")
        gx5b.plot([], [], "-", color="black", linewidth=2.8, label="Balanced (RF arc)")
        gx5b.plot([], [], "-", color="dodgerblue", linewidth=1.5, label="Takeoff Transition")
        gx5b.plot([], [], "-", color="seagreen", linewidth=1.5, label="Landing Transition")
    ...
    if best_rep is not None:
        rf_best = _apply_rf_corridor(best_rep)
        ...
        # ==================== 엑셀 저장 전 최적화된 회랑 경로 ====================
        # Transition(이륙/착륙) 직선 구간도 포함하여 저장한다.
        df_excel = df.copy()
