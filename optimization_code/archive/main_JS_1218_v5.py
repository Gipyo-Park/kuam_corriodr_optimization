...
def run_nsga3(
    ...
    for gen in range(1, Nmax + 1):
        ...
        if new_pop:
            ...
            # 현재 세대가 전부 실패면 이전 세대의 성공 해를 그대로 전달
            new_pop = list(last_success_pop)
            print(f"[Gen {gen}] no feasible in current generation; carrying over {len(new_pop)} previous feasible parent(s).")
...
def attempt_run_once():
    ...
    N_init = 1000    # 초기해 개수 
    N_pop = 100      # 세대별 유지할 개체 수(population size)
    Nmax = 10       # 최대 세대 수
    offspring_ratio = 0.5
    ...
    look_ahead_threshold_m = 1500.0  # 이보다 짧은 세그먼트 → R 스케일 다운 시작 (m)
    ...
    for _retry in range(1, max_init_retries + 1):
        ...
        if _both_cnt > 0:
            ...
            print(f"  -> {_both_cnt} RF+constraint feasible solution(s) found at retry {_retry}. Proceeding with feasible-only init pop.")
            break

    if init_pop is None:
        print(f"  Warning: No feasible init pop (>0) after {max_init_retries} retries. Restarting run.")
        return False, 0
