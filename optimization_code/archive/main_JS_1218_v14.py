...
def save_excel_route_map_figure(xlsx_path, out_png_path=None, figure_title=None):
    ...
    alt = pd.to_numeric(route_df.get("Alt_m"), errors="coerce")
    ...
    route["Alt_m"] = alt[valid].to_numpy(dtype=float)
...
def attempt_run_once():
    ...
    if len(reps) > 3:
        ...
        for config in risk_configs:
            ...
            gx_risk.set_title(f"Optimal Corridor ({config['name']}, RF Turn applied)")
    ...
    if best_rep is not None:
        ...
        try:
            save_excel_route_map_figure(
                ...
                figure_title="Balanced Optimal Corridor by Phase (from route_data.xlsx)",
            )
        except Exception as e:
