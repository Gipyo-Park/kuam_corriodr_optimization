...
def attempt_run_once():
    ...
    N_init = 100    # 초기해 생성 목표 개수(후속 필터를 통과한 해들 중 일부만 실제 init_pop으로 사용될 수 있음)
    ...
    local_radius_m = 350.0                # 현재 변이 지점 주변 반경(m)
    ...
    if reps:
        ...
        bal_path = rf_bal["path"]
        bal_full_path = build_full_corridor_path(start_vertiport, takeoff_complete, bal_path, landing_entry, end_vertiport)
        ...
        gx5.set_title("Balanced Corridor Only (RF Turn applied)")
        ...
        gx5b.set_title("Balanced Corridor Only (Fig4 Style)")
    ...
    if best_rep is not None:
        ...
        pt_noise_norm = _sample_point_noise(full_path, NoiseRisk, altitude_levels, lat_lim, lon_lim)
        pt_noise_db = _sample_point_noise(full_path, NoiseRiskDb, altitude_levels, lat_lim, lon_lim)
        df_path["Ground_Risk"] = pt_ground
        df_path["Air_Risk"] = pt_air
        df_path["Combined_Risk"] = pt_combined
        df_path["Noise_Risk_Normalized"] = pt_noise_norm
        df_path["Noise_Lden_dB_After_Floor"] = pt_noise_db
        ...
        df_summary = pd.DataFrame([
            ...
            {"Metric": "Total_Noise_Risk_Normalized", "Value": total_noise_risk_norm},
            {"Metric": "Total_Noise_Lden_dB_After_Floor", "Value": total_noise_db_after_floor},
            {"Metric": "Objective_NoiseRisk_Weighted", "Value": float(f_best[3]) if len(f_best) > 3 else np.nan},
            {"Metric": "Noise_Weight_w_noise", "Value": float(w_noise)},
            {"Metric": "Noise_Floor_dB", "Value": float(noise_floor_db)},
